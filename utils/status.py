...
async def check_first_line_status(status: str) -> (str, str, str):
    ...
    elif status == "reserve_delivery":
        return ["reserved_by_deliver", "awaiting_deliver", "deliver_id"]
    else:
        return ["", "", ""]


async def send_info_log(chat_id: int, action: str, ex_option: str = None) -> None:
    api = await sql.get_special_seller_api_info(chat_id)
    ...
    await dp.bot.send_message(chat_id=MODER_LOGS if DEBUG else api["log_chat_id"], text=text)
...
async def write_new_status(args: list, location: dict = None) -> None:
    extra_args = None
    if location:
